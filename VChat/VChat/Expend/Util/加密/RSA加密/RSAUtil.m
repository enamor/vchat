...
+ (NSString *)encryptString:(NSString *)str publicKey:(NSString *)pubKey{
    return [RSAUtil encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey];;
}
...
+ (NSString *)encryptData:(NSData *)data publicKey:(NSString *)pubKey{
    ...
    NSData *rsaData = [RSAUtil encryptData:data withKeyRef:keyRef];
    return [Base64 encode:rsaData];
}
...
+ (NSString *)decryptString:(NSString *)str privateKey:(NSString *)privKey{
    ...
    data = [RSAUtil decryptBase64String:str privateKey:privKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}

#pragma mark 私钥解密NSData
+ (NSData *)decryptBase64String:(NSString *)str privateKey:(NSString *)privKey{
    if(!str || !privKey){
        return nil;
    }
...
+ (NSData *)stripPublicKeyHeader:(NSData *)d_key{
	...
	// PKCS #1 RSAUtilEncryption szOID_RSAUtil_RSAUtil
...
+ (NSData *)stripPrivateKeyHeader:(NSData *)d_key{
	...
	if (!det) {
		...
		if (byteCount + idx > len) {
			//RSAUtil length field longer than buffer
			return nil;
		}
...
/* START: Encryption & Decryption with RSAUtil private key */
